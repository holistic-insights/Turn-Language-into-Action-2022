...
with tab1:
    ...
    if submit:
        ...
        with tab12:
            ...
            st.markdown(f'<h5>ESG categories counts</h5>', unsafe_allow_html=True)
            fig = px.bar(data_frame=cat_counts_df_all.sort_values(by='Category'), x='Category', y='Counts', text='Source', pattern_shape="Source", color='Category', color_discrete_sequence=['#B6E886', '#FF6692', '#19D3F3'])
            st.plotly_chart(fig, use_container_width=True)
